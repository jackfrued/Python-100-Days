...
> Note: When this repository was created in 2018, the content on the basics of Python was written relatively roughly, which might not be very friendly to beginners. If you happen to be a beginner, you might want to start with some other Python learning resources.
...
**History of Python**
  1. Christmas 1989: Guido van Rossum started writing the compiler for the Python language.
  2. February 1991: The first Python compiler (also an interpreter) was born, implemented in C, which could call C library     functions. Even in the earliest version, Python provided many core data types such as strings, lists, and dictionaries,     and had system calls and file I/O, as well as exception handling and functions.
  3. January 1994: Python 1.0 was officially released.
  4. October 16, 2000: Python 2.0 was released, which added garbage collection and many other features.
  5. December 3, 2008: Python 3.0 was released, which is not completely backward compatible with previous Python code. However, many companies still use Python 2.x versions in projects and operations, so many new features of Python 3.x are backported to Python 2.x.

Currently, I am using Python 3.7.x version, which was released in 2018. The version number of Python is divided into three segments, in the form of A.B.C. A represents the major version number, and when there is a major rewrite or non-backward compatible change, it will be incremented. B represents the minor version number, and when there are new features added, it will be incremented. C represents the micro version number, and when there are bug fixes or small improvements, it will be incremented.
...
Python has many advantages, which can be summarized as follows:
  ...
  1.Simple and clear, with a low learning curve, easier to get started than many programming languages.
  2.Open source, with a strong community and ecosystem, especially in the fields of data analysis and machine learning.
  3.Interpreted language with platform portability, code can work on different operating systems.
  4.Supports two main programming paradigms (object-oriented programming and functional programming).
  5.High code standardization and readability, suitable for people with code cleanliness and perfectionism.

Python's disadvantages mainly include:

  1.Slightly lower execution efficiency, but parts requiring high execution efficiency can be written in other languages       (e.g., C, C++).
  2.Code cannot be encrypted, but since many companies do not sell software but services, this issue is mitigated.
  3.Too many frameworks to choose from (e.g., there are over 100 web frameworks), and where there are choices, there are       mistakes.
