...
class Ball(object):
    ...
    def move(self, screen):
        ...
        self.x += self.sx
        self.y += self.sy
        if (self.x - self.radius <= 0 and self.sx < 0) or \
                (self.x + self.radius >= screen.get_width() and self.sx > 0):
            self.sx = -self.sx
        if (self.y - self.radius <= 0 and self.sy < 0) or \
                (self.y + self.radius >= screen.get_height() and self.sy > 0):
            self.sy = -self.sy
